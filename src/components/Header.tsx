  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center h-16 md:h-20">
        ...
        {/* Mobile Menu Trigger */}
        <button
          className="md:hidden relative w-8 h-8 flex flex-col justify-center items-center gap-1.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
          onClick={() => setMobileOpen(!mobileOpen)}
          aria-label={mobileOpen ? 'Закрити меню' : 'Відкрити меню'}
          aria-expanded={mobileOpen}
        >
          <span
            className={`block w-6 h-px bg-[var(--fg)] transition-all duration-300 origin-center ${
              mobileOpen ? 'rotate-45 translate-y-[3.5px]' : ''
            }`}
          />
          <span
            className={`block w-6 h-px bg-[var(--fg)] transition-all duration-300 ${
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed inset-0 top-16 bg-[var(--bg)] z-[99] flex flex-col items-center justify-center gap-10 md:hidden"
            role="dialog"
